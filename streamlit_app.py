...
if seleccion == "2015":
  ...
  with tab1:
    ...
    with metric_player:
        ...
        col = st.columns(2)
        col[1].metric("Posición", value=player["club_position"])
        col[2].metric("Número", value=int(player["club_jersey_number"]))
        col[1].metric("Salario Anual", value=int(player["wage_eur"]))
        col[2].image(player["club_logo_url"])
